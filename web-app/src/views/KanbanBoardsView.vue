<template>
  <div class="kanban-boards-view">
    <div class="header">
      <h1 class="title">Kanban доски</h1>
      <p class="subtitle">Управляйте своими Kanban досками</p>
    </div>

    <div class="content">
      <div class="section-header">
        <h2 class="section-title">Ваши Kanban доски</h2>
        <button
      ...
      <div class="boards-content">
        ...
        <div v-else-if="filteredBoards.length === 0" class="empty-state">
          <h3>Пока нет Kanban досок</h3>
          <p>Создайте свою первую Kanban доску, чтобы начать организовывать задачи</p>
          <button
            @click="showCreateModal = true"
        ...
        <div v-else class="boards-grid">
          ...
          >
            ...
            <div class="board-stats">
              ...
              <div class="stat">
                <span class="stat-value">{{ board.columns.length }}</span>
                <span class="stat-label">Столбцы</span>
              </div>
            </div>
...
const filteredBoards = computed(() => 
  boards.value.filter(board => board.type === 'kanban')
)
...
function openBoard(board: Board) {
  console.log('Opening Kanban board:', board.id)
  router.push({ name: 'Kanban', params: { boardId: board.id } })
}

async function handleCreateBoard(boardData: CreateBoardRequest) {
  try {
    // Проверка, что тип доски установлен как kanban
    const kanbanBoardData = {
      ...boardData,
      type: 'kanban' as const
    }
    await taskStore.createBoard(kanbanBoardData)
