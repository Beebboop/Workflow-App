  ...
  describe('Тестирование решений в методе deleteTask', () => {
    ...
    const decisionTable = [
      ...
      },
      {
        ...
        isReporter: true, 
        expectedResult: 'NotFoundException',
        description: 'Задача не существует (альтернативный сценарий(isReporter)) (true/false)'
      },
    ];

    decisionTable.forEach(({ testCase, taskExists, isReporter, expectedResult, description }) => {
      it(`Тестовый случай ${testCase}: ${description}`, async () => {
        ...
        if(task === null){
            ...
            it.skip(`Тестовый случай ${testCase}: ${description}`);
        }
