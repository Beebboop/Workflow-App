...
describe('AuthService - Boundary Value Analysis', () => {
  ...
  describe('Граничные тесты(BVA) на длину пароля', () => {
    ...
    describe('Проверка конкретных сообщений об ошибках', () => {
        const testCases = [
            {
            description: 'Пароль 3 символа',
            password: '123',
            expectedMessage: 'Password must be at least 4 and 20 maximum characters long'
            },
            {
            description: 'Пароль 21 символ',
            password: 'a'.repeat(21),
            expectedMessage: 'Password must be at least 4 and 20 maximum characters long'
            },
            {
            description: 'Имя 3 символа после trim',
            name: 'abc',
            expectedMessage: 'Name must be at least 4 and 20 maximum characters long'
            },
            {
            description: 'Имя 21 символ',
            name: 'a'.repeat(21),
            expectedMessage: 'Name must be at least 4 and 20 maximum characters long'
            },
            {
            ...
            setup: () => usersRepository.findOne.mockResolvedValue({ id: 'existing' }),
            expectedMessage: 'User with this email already exists'
            }
        ];

        testCases.forEach(({ description, password, name, email, setup, expectedMessage }) => {
            it(`должен возвращать "${expectedMessage}" для ${description}`, async () => {
            if (setup) setup();
